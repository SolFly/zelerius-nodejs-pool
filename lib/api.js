...
function handleServerRequest(request, response) {
    ...
    switch(urlParts.pathname){
        ...
        case '/admin_profit':
            ...
            handleAdminProfit(urlParts,response);
            break;
...
function handleAdminProfit(urlParts,response){
    ...
    async.waterfall([
        ...
        function(callback) {
          ...
          redisClient.zrange(config.coin + ":REGISTER:profit:sent", (-1)*(urlParts.query.elements || 10), -1, function(error, redisData) {
            var profitData = {};
            for(var i in redisData) {
              var data = redisData[i].split(':');
              //var hash = keyParts[keyParts.length-2];
              //var data = redisData[i];
              profitData[data[5]] = {
                  amount: data[0],
                  fee: data[1],
            ...
            callback(false, profitData);
          });
        }
        ], function(error, profitData) {
            if(error) {
                response.end(JSON.stringify({error: 'Error collecting profit payments'}));
                return;
            }
            response.end(JSON.stringify(profitData));
        }
    );
}
...
function getCNv()// I'm sure that I can do this better...
{
  getLastBlockData(function(error, data) {
	  //log('info', logSystem, 'Getting cnVariant %j -- %j', [data, config.V6]);
	  daemonFinished = Date.now();
    if(typeof data !== 'undefined' && 'height' in data) {
  	  if(typeof config !== 'undefined' && config.V7){
  	    if(data.height + 1 > config.V7) //height of lastblock then +1
  	      cnV = 11; // CN variant zls
        else
  	      cnV = 8; // CN variant 2
  	  }
	  }
    else {
      log('error', logSystem, 'Error getting cnVariant');
    }
  });
  return cnV;
}
