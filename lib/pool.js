/**
*
*  _____ _____  _      _____  ____   ___  _   _  ____
* |__  /| ____|| |    | ____||  _ \ |_ _|| | | |/ ___|
*   / / |  _|  | |    |  _|  | | ) | | | | | | |\___ \
*  / /_ | |___ | |___ | |___ | |\ <  | | | |_| | ___) |
* /____||_____||_____||_____||_| \_\|___| \___/ |____/
*
*	ZELERIUS NETWORK
...
BlockTemplate.prototype = {
    ...
    nextBlob: function(){
        ...
        var blob_current_version = utils.cnUtil.convert_blob(this.buffer, cnBlobType).toString('hex');

	//log('info', logSystem, 'nextBlob 1 - %j', [blob_current_version]);

	/*blob_current_version = blob_current_version.replaceAt(0, this.buffer.toString('hex')[0] + this.buffer.toString('hex')[1]);//Major version
	blob_current_version = blob_current_version.replaceAt(2, this.buffer.toString('hex')[2] + this.buffer.toString('hex')[3]);//Minor version

	log('info', logSystem, 'nextBlob 2 - %j', [blob_current_version]);*/

	return blob_current_version;
    },
...
function jobRefresh(loop, callback){
    ...
    getBlockTemplate(function(error, result){
        ...
        //log('info', logSystem, 'buffer 1 - %j', [buffer]);
        buffer.copy(new_hash, 0, 7, 39);
        //log('info', logSystem, 'new_hash 2 - %j', [new_hash]);
        ...
        /*
        if(config.V7 && result.height > config.V7){
           cnVariant = 11; // CN variant zls
        }
        else {
           cnVariant = 8; // CNv2
         }
...
Miner.prototype = {
    ...
    getJob: function(){
        ...
        if (!this.proxy) {
            ...
            //log('info', logSystem, 'blob - %j', [blob]);
            //log('info', logSystem, 'nextBlobWithChildNonce - %j', [currentBlockTemplate.nextBlobWithChildNonce()]);
            //blob = currentBlockTemplate.nextBlobWithChildNonce();
...
function processShare(miner, job, blockTemplate, params){
    ...
    if (shareTrustEnabled && miner.trust.threshold <= 0 && miner.trust.penalty <= 0 && Math.random() > miner.trust.probability){
        ...
        //hash = cryptoNight(convertedBlob, hard_fork_version >= 7 ? 11 : 8);// CNvarint2 = 8 CNvariantZLX = 11 Zelerius V6 is CNv2 Zelerius V7 is CNvZLS
        hash = cryptoNight(convertedBlob, cnVariant); //ZLS
        log('info', logSystem, 'Mining pool algorithm: %s variant %d, Hard fork version: %d', [cnAlgorithm, cnVariant, shareBuffer[0]]);
        shareType = 'valid';
    }
