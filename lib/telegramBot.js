...
function sendMinerStats(telegramMsg, address) {
    ...
    {
        var plusURL = '%2B';//+ is equal to %2B in URL link
        address = address.split(' ').join(plusURL);
        log('info', logSystem, 'space replaced by %2B (+)');
    }
    ...
    apiInterfaces.pool('/stats_address?address='+address, function(error, data) {
        ...
        {
                var temporal = { hashrate: 0, balance: 0, paid: 0, lastShare: 0};
                data.stats = temporal;
        }           
...
function toggleMinerNotifications(telegramMsg, address) {
    ...
    if(address.indexOf(' ') > -1)
    {
        log('error', logSystem, 'Unable to get API data for miner stats');
        return bot.sendMessage(telegramMsg.from.id, 'Unable to get miner statistics. Please retry.' + address);
    }
    apiInterfaces.pool(apiRequest, function(error, response) {
